...
#define BAT_LENGTH 45
#define BALL_LENGTH 15

// define the 8bit colour constants for the game pieces
#define BAT_COL 0x1F 	//cyan
#define BALL_COL 0xF0 	// orange

// These are the limits of the game area on screen
// x limits are in CPU ticks, there are about 2 ticks per pixel
#define MINX 0
#define MAXX 1200
#define MINY 50
#define MAXY 400
...
void setup() {
  ...
  bat1 = 512/3+50;	// start bat1 in the middle of the screen
  bat2 = 512/3+50;

  ball = 512/3+50;		// start in centre like a serve

  ball_x = 0x400;	// somewhere near the middle of the screen
...
void loop() {
  ...
  if(ball_x >= MAXX) {
    if((ball > bat2-BALL_LENGTH) && (ball < (bat2 + BAT_LENGTH))) {
      ...
      // where you hit it
      ball_y_dir = (bat2 - ball + (BAT_LENGTH / 2)) * -3 / BAT_LENGTH;
    } else {
    ...
    if((ball > bat1-BALL_LENGTH) && (ball < (bat1 + BAT_LENGTH))) {
      // ball hit bat1
      ball_x_dir *=-1;
      ball_y_dir = (bat1 - ball + (BAT_LENGTH / 2)) * -3 / BAT_LENGTH;
    } else {
